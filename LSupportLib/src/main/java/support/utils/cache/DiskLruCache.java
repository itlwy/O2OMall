...
public class DiskLruCache {
    ...
    private final Map<String, String> mLinkedHashMap = Collections.synchronizedMap(new LinkedHashMap<String, String>(INITIAL_CAPACITY,
            LOAD_FACTOR, true));
	private static final int IO_BUFFER_SIZE = 8 * 1024;  //写bitmap进文件时用的缓存大小
    ...
    public static DiskLruCache openCache(Context context, File cacheDir, long maxByteSize) {
        ...
        if (cacheDir.isDirectory() && cacheDir.canWrite() && SystemUtils.getAvailableSpace(cacheDir) > maxByteSize) {
    ...
    public static File getDiskCacheDir(Context context, String uniqueName) {
        ...
        final String cachePath = Environment.getExternalStorageState() == Environment.MEDIA_MOUNTED || !SystemUtils.isExternalStorageRemovable() ? SystemUtils
